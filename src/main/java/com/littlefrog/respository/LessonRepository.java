...
public interface LessonRepository extends JpaRepository<Lesson,Integer>{
    ...
    @Query (value = "SELECT * from lesson u where u.lesson_id = ?1",nativeQuery = true)
    public Lesson findLessonByLessonID(int lesson_id);

    @Transactional
    @Modifying
    @Query(value = "UPDATE lesson SET title = ?2 where lesson_id = ?1",nativeQuery = true)
    public void setTitle(Integer Id, String title);

    @Transactional
    @Modifying
    @Query(value = "UPDATE lesson SET video_url = ?2 where lesson_id = ?1",nativeQuery = true)
    public void setVedioUrl(Integer Id, String Url);

    @Transactional
    @Modifying
    @Query(value = "UPDATE lesson SET description = ?2 where lesson_id = ?1",nativeQuery = true)
    public void setDescription(Integer Id, String description);

    @Transactional
    @Modifying
    @Query(value = "UPDATE lesson SET cover_pic = ?2 where lesson_id = ?1",nativeQuery = true)
    public void setCover_url(Integer Id, String cover_url);

    @Transactional
    @Modifying
    @Query(value = "UPDATE lesson SET title =  ?2, video_url=?3, description=?4, cover_pic=?5 where lesson_id = ?1",nativeQuery = true)
